...
class LogAnalysisComparison
{
    ...
    public function isAnyNewError(): bool
    {
        return $this->differences->error > 0;
    }

    public function isAnyNewWarning(): bool
    {
        return $this->differences->warning > 0;
    }

    public function isAnyNewCritical(): bool
    {
        return $this->differences->critical > 0;
    }

    public function isAnyNewAlert(): bool
    {
        return $this->differences->alert > 0;
    }

    public function isAnyNewEmergency(): bool
    {
    ...
    {
        return $this->isAnyNewError()
            || $this->isAnyNewWarning()
            || $this->isAnyNewCritical()
            || $this->isAnyNewAlert()
            || $this->isAnyNewEmergency();
    }
